...
`sheet`: `{"sheet": n, "dpi": d, "out": "<png path>"}` renders 1-based sheet `n` into `out` (replaced whole).
...
The LAST stdout line is `{"ok": true, ...}` or `{"ok": false, "code": "internal"}` (the engine's lazy
