...
existing CLI test rather than adding one).
...
- **The smoke's XFF assertion** is the `429` on the third spoofed upload plus the `rate` table's distinct-hash count read by
  ...
  hold whether or not the caddy path's client resolves to the same peer as the direct path (it does here).
...
- **Domain** (Q-1): registered, with DNS access; an `A` (and `AAAA` if the VM has v6) record → the VM's public IP BEFORE
  the first `up`, or Caddy's ACME order fails and retries with back-off.
...
docker already uses `172.30.0.0/24`, set `PDFSPLIT_SUBNET`/`PDFSPLIT_CADDY_IP` together. The uptime check (AC-5 of 014)
